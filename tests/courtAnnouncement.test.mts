...
test("builds one court announcement", () => {
  ...
  assert.equal(text, "십오번 코트, 준호, 재아, 윤철, 혜지 들어가세요.");
});
...
test("builds all ready court announcements and skips incomplete courts", () => {
  ...
  assert.equal(text, "전체 코트 안내입니다. 십오번 코트, 준호, 재아, 윤철, 혜지 들어가세요.");
});
...
test("uses the edited numeric court name when announcing", () => {
  ...
  assert.equal(text, "이십사번 코트, 준호, 재아, 윤철, 혜지 들어가세요.");
});
...
test("uses the edited text court name when announcing", () => {
  ...
  assert.equal(text, "메인 코트, 준호, 재아, 윤철, 혜지 들어가세요.");
});
...
test("pronounces VIP1 and VIP2 in English", () => {
  ...
  assert.equal(vip1, "VIP one 코트, 준호, 재아, 윤철, 혜지 들어가세요.");
  assert.equal(text, "VIP two 코트, 준호, 재아, 윤철, 혜지 들어가세요.");
});
